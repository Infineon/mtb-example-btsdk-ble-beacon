# BLE Beacon Sample application

## Overview
This app demonstrates use of Google Eddystone and Apple iBeacons via the
beacon library. It also demonstrates uses of the multi-advertisement feature.

During initialization the app configures advertisement packets for Eddystone and iBeacon, and starts advertising via multi-advertisement APIs. It also sets up a 1 sec timer to update Eddystone TLM advertisement data.
...
2. Monitor advertisement packets using any of the methods below:
    ...
    - On iOS, download an app such as 'Physical Web or 'Locate Beacon'. In the 'Locate Beacon' phone app, enter the UUID for iBeacon. (see UUID\_IBEACON in the application file beacon.c for the UUID definition).
    - or use an over the air sniffer
3. Run the BTSpy app, capture protocol and snoop traces and view the traces via a viewer such as Frontline.

## Notes
This application supports OTA Firmware Upgrade. To update the application,
see the readme in wiced\_btsdk\tools\btsdk-peer-apps-ota. Also see the note at the end
